# -*- coding: utf-8 -*-
"""
Created on Sat Jun 20 10:54:13 2020

@author: yan10
"""


        i = np.floor(nxs).astype(int)+1
        j = np.floor(nys).astype(int)+1
        
        px = (nxs-i)
        py = (nys-j)
        
        
        f00 = self.im[i-1, j-1]      #row0 col0 >> x0,y0
        f01 = self.im[i-1, j]      #row0 col1 >> x1,y0
        f02 = self.im[i, j+1]      #row0 col2 >> x2,y0
        f10 = self.im[i-1, j]      #row0 col1 >> x1,y0
        f20 = self.im[i, j+1]      #row0 col2 >> x2,y0
        f13 = self.im[i, j+2]      #row0 col1 >> x1,y0
        f23 = self.im[i+1, j]+2      #row0 col1 >> x1,y0
        f31 = self.im[i+2,j]         #row3 col1 >> x1,y3
        f32 = self.im[i+2,j+1]       #row3 col2 >> x2,y3
        f03 = self.im[i-1, j+2]      #row0 col3 >> x3,y0
        f30 = self.im[i-1, j+2]      #row3 col0 >> x0,y3
        f33 = self.im[i+2,j+2]       #row3 col3 >> x3,y3
